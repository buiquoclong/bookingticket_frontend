...
const AdminContact = () => {
    ...
    async (searchDebounce) => {
      try {
        const response = await fetch(
          `http://localhost:8081/api/contact/page?page=${page}&size=10&email=${searchDebounce}`
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetchContacts(searchDebounce);
    ...
    fetchData();
  }, [page, searchDebounce, fetchContacts]);
  ...
  const handleUpdateContact = async (e) => {
    ...
    if (missingInfo.length > 0) {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:8081/api/contact/${currentContact.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const removeContact = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const updatedContact = records.filter(
  ...
  return (
    <div className="main-container">
      ...
      <Breadcrumbs aria-label="breadcrumb">
        ...
        <Link underline="hover" color="inherit" href="/admin/contacts">
          Liên hệ
        </Link>
      </Breadcrumbs>
